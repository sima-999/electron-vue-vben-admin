...
class CoreLoader {
  ...
  constructor() {
    ...
    if (!_.isString(CoreLoader.libPath) || !existsSync(CoreLoader.libPath)) {
      logger.error(
        `[CoreLoader] Update resource folder: ${CoreLoader.libPath} --> ${CoreLoader.defaultLibPath}`,
      );
  ...
  public GetImage(uuid: string): string {
    ...
    const len = this.func.AsstGetImage(this.GetCoreInstanceByUUID(uuid), buffer, buffer.length);
